    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
            ...
            <motion.div
                ...
                transition={{ duration: 0.7 }}
                className="col-span-1 lg:col-span-5 relative z-10 text-right"
            >
                <h3 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white mb-6">
                    {name}
                </h3>
                
                <div className="mb-6 p-6 bg-white dark:bg-white/5 rounded-2xl border border-slate-100 dark:border-white/10 shadow-lg dark:shadow-none backdrop-blur-sm ml-auto">
                ...
                <div className="flex flex-wrap gap-2 mb-8 justify-end">
                    {tech?.map((item, i) => (
                        <span key={i} className="px-3 py-1 bg-slate-100 dark:bg-purple-500/10 text-slate-600 dark:text-purple-300 rounded-full text-sm font-medium border border-slate-200 dark:border-purple-500/20">
                            {item}
                        </span>
                    ))}
                </div>
                ...
                <div className="flex gap-4 justify-end">
                    <a href={source} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-slate-600 dark:text-slate-300 hover:text-purple-600 dark:hover:text-white transition-colors">
                        <FiGithub className="w-5 h-5" /> <span>Source Code</span>
                    </a>
                    {demo && (
                        <a href={demo} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white rounded-lg shadow-lg hover:shadow-purple-500/30 transition-all">
